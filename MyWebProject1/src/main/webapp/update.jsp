...
%> --%>
<%@ page import="DAO.process, Model.Employee, java.sql.Date" %>
<%-- Correct taglib declaration --%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%
    String idS = request.getParameter("Id");
    if (idS == null) { response.sendRedirect("index.jsp"); return; }
    int id = Integer.parseInt(idS);
    Employee updates = new Employee();
    if (request.getParameter("update_firstName") != null) {
        updates.setFirstName(request.getParameter("first_Name"));
    ...
    if (request.getParameter("update_age") != null) {
        String a = request.getParameter("Age");
        if (a != null && !a.isEmpty()) updates.setAge(Integer.parseInt(a));
    }
    if (request.getParameter("update_join") != null) {
        String jd = request.getParameter("Join_Date");
        if (jd != null && !jd.isEmpty()) updates.setJoinDate(Date.valueOf(jd));
    }
    try {
        process dao = new process();
        dao.updateById(id, updates);
        request.setAttribute("message", "Updated record with Id = " + id);
        request.getRequestDispatcher("success.jsp").forward(request, response);
    } catch(Exception ex) {
        ex.printStackTrace();
        out.println("<h3>Error: " + ex.getMessage() + "</h3><a href='index.jsp'>Back</a>");
    }
%>
